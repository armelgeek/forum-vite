...
const DeleteAction = ({ item, rerender }: any) => {
    const destroy = useDispatch('theme', 'destroy');
...
const index = () => {
    ...
    const meta = useGetter('theme', 'meta', []);
    const fetch = useDispatch('theme', 'fetch');
                        ...
                        >
                            <div className='flex flex-row items-center gap-3'>
                               ...
                               {/**<div className='ml-3'><Link to={`/admin/theme/detail/${row.original.id}`} className="btn bg-primary-500 btn-xs text-white flex flex-row items-center  gap-1"><FaInfoCircle size={14} /><p className='text-sm'>Détail</p> </Link></div>**/}
                                <div><Link to={`/admin/theme/edit/${row.original.id}`} className="btn flex flex-row items-center  bg-secondary-500 btn-xs text-white  gap-1"><FaPencil size={12} /><p className='text-sm'>Editer</p> </Link></div>
    ...
    return (
        <Page>
            <Links title={"Themes de discussions"} description={"Ici vous pouvez faire la gestion de tous vos themes de discussions"}>
                <ActiveLink title={"Themes de discussions"}></ActiveLink>
            </Links>
            <div className="flex flex-row justify-between items-center py-3">
                ...
                <div className="action">
                    <Link to={"/admin/theme/add"} className='btn btn-sm text-white bg-primary-500'><FaPlusCircle /> Ajouter un theme</Link>
                </div>
            </div>
